...
FORMA — сервис, где пользователь выбирает дизайн обложки блокнота из каталога курированных коллекций или генерирует свой через промпт/теги, выбирает уровень исполнения (Core / Signature / Lux), наполнение, и заказывает с оплатой через ЮKassa. Два пути: каталог (Training Pipeline) и генерация (пользовательская). Бренд = "редакция вкуса", не "кастом-печать".

## Architecture Location

```
docs/v0.2/              — текущая версия спецификации (7 файлов)
docs/v0.1/              — предыдущая версия (архив, не использовать)
docs/PLAN.md            — пошаговый план реализации (Фаза 1: этапы 0-8, Фаза 2: этапы 9-16)
...
Документация = 7 файлов, ~4000 строк. Загрузка всех убивает контекст. Работай по принципу **minimum context**:
...
|----------------------------------------------|-------------------------------|
| Стек, роли, бренд, тиры, термины            | `docs/v0.2/01-overview.md`    |
| UX-флоу, экраны S0-S17, переходы            | `docs/v0.2/02-user-stories.md`|
| Каталог, коллекции, дропы, поиск, шаринг    | `docs/v0.2/03-public.md`      |
| Админка: теги, стили, Training Pipeline      | `docs/v0.2/04-admin.md`       |
| Генерация, тиры, мутации, провайдеры        | `docs/v0.2/05-generation.md`  |
| Схема БД, таблицы, индексы                  | `docs/v0.2/06-storage.md`     |
| ЮKassa, штрихкоды, лимиты, limited edition  | `docs/v0.2/07-orders-payments.md` |
| Что делать следующим                        | `docs/PLAN.md`                |

### Rule 3: PLAN.md is the task list

`docs/PLAN.md` содержит 16 этапов (Фаза 1: 0-8, Фаза 2: 9-16) с чеклистом. При начале работы:
1. Открой `PLAN.md`
...
`manual.md` — сырой исходник, из которого создана документация. **Никогда не используй его как источник истины.** Все решения зафиксированы в `docs/v0.2/`. Если есть расхождение — `docs/v0.2/` побеждает. `docs/v0.1/` — архив, не использовать.
...
- **Design** — центральная сущность (дизайн блокнота), может принадлежать Collection
- **Generation** → **GenerationVariant** (kind × tier): kind (main/mutation_a/mutation_b) × tier (core/signature/lux, nullable)
- **Tag** (public/hidden) + **TagCategory** + **TagRelation** (совместимость)
- **Style** — карточка каталога с пресетом генерации
- **Collection** — группировка дизайнов (regular / limited с stock tracking)
- **Drop** — лимитированный выпуск с нумерацией экземпляров (N/300)
- **TierModifier** — промпт-модификаторы для Core/Signature/Lux
- **Order** → **OrderItem** (с tier snapshot) + **Payment** (ЮKassa) + **OrderFile** (PDF для печати)
- **NotebookSku** — Core / Signature / Lux (цены TBD)
- **Filling** — внутренний блок (клетка/линейка/точки/пустые)
- **GenerationPass** — безлимит за 100 ₽ на 24 часа
- **TrainingBatch** + **ReferenceImage** — Training Pipeline (загрузка → AI-анализ → курирование → генерация)
...
TrainingBatch:   created → analyzing → analyzed → generating → completed | failed
ReferenceImage:  uploaded → analyzing → analyzed → curating → curated → generating → generated → published | rejected
```

### Два пайплайна генерации

**1. Пользовательская генерация** (без tier):
```
"Сгенерировать" нажата
  → Generation (source: user)
  → 3x GenerationVariant (main, mutation_a, mutation_b; tier: NULL)
...
**2. Training Pipeline** (с tier):
```
Загрузка фото → AI-анализ (Claude Vision + GPT-4V)
  → Курирование (выбор промпта, теги, коллекция)
  → Generation (source: training_pipeline)
  → До 9x GenerationVariant (kind × tier)
  → PromptComposer: curated_prompt + tier_modifier + identity_elements
...
Сущности с человекочитаемыми URL: styles, tags, tag_categories, designs, drops, fillings, catalog_sections, collections.
...
8. **Не используй `manual.md` или `docs/v0.1/` как источник истины.** Только `docs/v0.2/`.
9. **Не создавай миграцию с `float`/`decimal` для денег.** Только `integer` + `_cents` суффикс.
10. **Не путай тиры.** Core / Signature / Lux (не Base / Pro / Elite). Разница производственная, не эстетическая.
...
forma/
  ...
  app/
    ...
    services/
      ...
      training/
        ai_analyzer.rb           — AI-анализ референсов (Claude Vision + GPT-4V)
        batch_processor.rb       — обработка пачек референсов
      payments/
...
Rails-приложение создано, Этапы 0-8 (Фаза 1 / v0.1) выполнены. Документация v0.2 готова. Следующий шаг — **Этап 9** из `docs/PLAN.md` (Tier-система и TierModifier).
